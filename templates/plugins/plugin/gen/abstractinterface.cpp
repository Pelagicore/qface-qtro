...
/*!
   \qmlmethod {{operation.type}} {{interface}}::{{operation}}({{operation.parameters|map('qt.parameterType')|join(', ')}})
{% with doc = operation.comment|parse_doc %}
   \brief {{doc.brief}}
   {{doc.description}}
{% endwith %}
*/
{{operation|qt.returnType}} {{class}}::{{operation}}({{operation.parameters|map('qt.parameterType')|join(', ')}})
{
